...
def evolve(population, target, minimum, maximum, count, retain, mutation):
	tupled = [ (fitness(individual, target), individual) for individual in population ] # Create pairs (fitness, individual)
	# Select individuals with an approach of the "pool method" to maintain genetic diversity.
	parents = []
	counter = 1
	while len(parents) <= retain*count:
		# print(len(tupled))
		# print(count-1)
		# print(retain*count)
		# print(tupled[95:])
		# print(tupled[-1])
		# print("------------------")
		selected = tupled[randint(0, count - counter)]
		tupled.remove(selected)
		if randint(0, maximum) < selected[0]:
...
counter = 1
while evaluation > 0:
	pop = evolve(pop, target, minimum, maximum, count, retain, mutation)
	evaluation = evaluate(pop, target)
